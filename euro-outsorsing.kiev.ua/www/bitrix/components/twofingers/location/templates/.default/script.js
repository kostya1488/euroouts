...
{
    var TFLocationOverlay   = $('.custom-popup-2014-overlay'),
    ...
    TFLocationCallback      = callback;
    TFLoctaionComponentPath = path;
            ...
            $.get(path+'/functions.php', {request: 'getcities'}, function(data)
            {
                ...
                if (data.CITIES) {
                    $.each(data.CITIES, function(key, city) {
                        ...
                        location = '<li><a class="tf-location__link" data-id="'+city.ID+'" href="#">'+city.NAME+'</a>';

                        TFLocationCitiesList.find('ul').append()
                        ...
                        TFLocationCitiesList.find('ul').append(location)
                    });
                }
                if (data.DEFAULT_CITIES) {
                    $.each(data.DEFAULT_CITIES, function(key, city) {
                        TFLocationCurrentList.append('<li><a data-id="'
                            + city.ID + '" href="#">'
                            + city.NAME + '</a></li>');
                    });
                } else {
                ...
                TFLocationPopupContent.fadeIn(function()
                {
                    ...
                    {
                        var selctedCityID   = $(this).data('id'),
                            selctedCityNAME = $(this).html(),
                            $route          = $('.bx-ui-sls-route');

                        if ($route.length) {
                            $route.val(selctedCityNAME);
                        }
                        ...
                        $.post(TFLoctaionComponentPath+'/functions.php', {request: 'setcity', city: selctedCityID}, function()
                        {
                            ...
                            TFLocationPopup.fadeOut(speed, function() {
                                ...
                                TFLocationCallback = TFLocationCallback.replace('#TF_LOCATION_CITY_ID#', 'selctedCityID');
                                TFLocationCallback = TFLocationCallback.replace('#TF_LOCATION_CITY_NAME#', 'selctedCityNAME');
